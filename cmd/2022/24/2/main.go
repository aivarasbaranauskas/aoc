...
func main() {
	...
	//printWinds(winds)
	forward := aStar(State{1, 0, 0}, width-2, height-1)
	back := aStar(State{width - 2, height - 1, forward}, 1, 0)
	forwardAgain := aStar(State{1, 0, back + forward}, width-2, height-1)
	fmt.Println(forward, back, forwardAgain)
	fmt.Println(forward + back + forwardAgain)
}
...
func aStar(start State, goalX, goalY int) int {
