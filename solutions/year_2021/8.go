...
func (d Day8) Part2(input []byte) string {
	...
	for r.Scan() {
		...
		// find 5
		for p := range p235 {
			if d.countBitsSet(patterns[4]&p) == 3 {
				patterns[5] = p
				delete(p235, p)
				break
			}
		...
		// find 6
		for p := range p069 {
			if d.countBitsSet(patterns[1]&p) == 1 {
				patterns[6] = p
				delete(p069, p)
				break
			}
		...
		// find 9
		for p := range p069 {
			if d.countBitsSet(patterns[4]&p) == 4 {
				patterns[9] = p
				delete(p069, p)
				break
			}
		...
		output := patternsMap[d.signalsBin(outputSignals[0])]*1000 + patternsMap[d.signalsBin(outputSignals[1])]*100 + patternsMap[d.signalsBin(outputSignals[2])]*10 + patternsMap[d.signalsBin(outputSignals[3])]
		sum += output
	}
...
func (Day8) countBitsSet(input uint8) (ct int) {
	for i := 0; i < 8; i++ {
		if input&(1<<i) == 1<<i {
			ct++
		}
...
func (Day8) signalsBin(input string) uint8 {
	var pattern uint8
